...
```{r chargement_donnees, message=FALSE, warning=FALSE}
library(readr)
library(sf)
...
```{r altitude_finale, message=FALSE, warning=FALSE}
library(terra)
library(geodata)
library(sf)
...
```{r raster_final_titre_complet, message=FALSE, warning=FALSE}
library(terra)
library(sf)
...
```{r stats_comparatives, message=FALSE, warning=FALSE, results='hold'}
library(sf)         
library(dplyr)       
library(knitr)       
library(kableExtra)  
...
tm_shape(carreaux_map) +
  ...
  tm_layout(main.title = "Carte de la distribution spatiale des revenus", 
            main.title.size = 1.2,                                    
